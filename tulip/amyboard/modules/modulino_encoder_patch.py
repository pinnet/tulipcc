# modulino_encoder_patch.py — frozen into firmware (boards/manifest.py) and
# auto-imported by amyboard._load_user_patches() before the sketch runs, so
# this patch applies on every boot regardless of what /user/current/sketch.py contains.
#
